...
Handles pages at onlinelibrary.wiley.com.
"""
...
class WileyScraper(BaseScraper):
    """Scraper for Wiley (onlinelibrary.wiley.com)."""
    ...
    async def _scrape_async(
        ...
        async with self._browser_tab(cookies_file) as tab:
            ...
            print(f"  ▸ Fetching Wiley page: {nav_url}")
            ...
            try:
                await tab.query('.citation__title, h1.article-header__title', timeout=15)
            except Exception:
                pass
            ...
            # Title
            title_el = self._first(page.css('.citation__title, h1.article-header__title'))
            paper.title = self._clean_text(self._get_text(title_el)) if title_el else ""

            # Authors
            author_els = page.css('.author-name span, .accordion-tabbed__tab-mobile')
    ...
    async def _extract_sections(
        ...
        top_sections = body_el.css("section, .article-section")
    ...
    async def _extract_single_section(
        ...
        heading_el = self._first(sec_el.css("h2, h3, h4, .article-section__title"))
        if not heading_el:
            return None
    ...
    async def _extract_figure(
        ...
        caption_el = self._first(element.css("figcaption, .figure__caption"))
